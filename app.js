...
function renderAll() {
    ...
    for (let h = 0; h < 24; h++) {
        for (let m = 0; m < 60; m += 30) {
            ...
            div.onclick = () => { 
                if(isLocked && !adminAuthenticated) return alert("예약 마감되었습니다. / Locked."); 
                selectedSlot = id; 
...
function confirmBooking() {
    ...
    if(!a || !p || !idNum || !d || !pass) return alert("모든 항목 필수! / Fill all fields.");
...
function confirmCancel() {
    ...
    ref.get().then(doc => {
        ...
        ref.update({ attendees: list }).then(() => { closeReservedModal(); addLog(`Cancelled: ${mine.player} (${selectedSlot})`); alert("취소됨."); });
    });
}
...
function exportAllCSV() {
    try {
        ...
        if (!hasData) return alert("데이터 없음."); XLSX.writeFile(wb, `SVS_Export.xlsx`); addLog("Excel Exported");
    } catch (e) { alert("Fail."); }
}

function handleAdminAccess() { sc++; if(sc>=3) { sc=0; var p=prompt("Pass:"); if(p==="2737") { adminAuthenticated=true; document.getElementById("adminPanel").classList.add("show"); addLog("Admin Logged In"); updateAdminUI(); } } }
...
function backupAndClearAll() { if(!confirm("전체 삭제?")) return; db.collection("slots").get().then(snap => { var batch = db.batch(); snap.forEach(doc => batch.delete(doc.ref)); batch.commit().then(() => { addLog("All Data Cleared"); alert("Cleared."); }); }); }
...
function deleteAttendee(slotId, index) { var ref = db.collection("slots").doc(slotId); ref.get().then(doc => { var list = doc.data().attendees; var pName = list[index].player; list.splice(index, 1); ref.update({ attendees: list }).then(() => { addLog(`Admin deleted: ${pName}`); openReservedModal(slotId); }); }); }
