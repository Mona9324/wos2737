...
function renderAll() {
    ...
    for (let h = 0; h < 24; h++) {
        for (let m = 0; m < 60; m += 30) {
            ...
            div.onclick = () => { 
                if(isLocked && !adminAuthenticated) return alert("예약 마감됨 / Locked."); 
                selectedSlot = id; 
...
function confirmBooking() {
    ...
    if(idNum.length !== 9 || isNaN(idNum)) return alert("ID는 반드시 9자리 숫자여야 합니다. / ID must be 9 digits.");
    ...
    .then(() => { 
        // ID까지 로컬스토리지에 저장 (영구 유지)
        localStorage.setItem(MY_BOOKING_KEY, JSON.stringify({ 
            alliance: a, player: p, playerId: idNum, cancelKey: pass 
        })); 
...
function confirmCancel() {
    ...
    ref.get().then(doc => {
        ...
        if(list.length === doc.data().attendees.length) return alert("비밀번호 오류! / Wrong password.");
        ref.update({ attendees: list }).then(() => { 
            closeReservedModal(); addLog(`Cancelled: ${mine.player} (${selectedSlot})`); 
            document.getElementById("editCancelKey").value = ""; alert("취소됨 / Cancelled."); 
        });
    });
}
...
function saveAdminBaseDate() { var val = document.getElementById("adminBaseDate").value; if(!val) return; db.collection("settings").doc("booking").update({baseDate: val}).then(()=>{ addLog("SVS Date Updated"); alert("Saved"); }); }
function toggleTabStatus(day) { var current = bookingSettings.tabs[day].isOpen; bookingSettings.tabs[day].isOpen = !current; db.collection("settings").doc("booking").update(bookingSettings).then(() => addLog(`${day} Status Changed`)); }
function toggleAllTabs(status) { Object.keys(bookingSettings.tabs).forEach(k => bookingSettings.tabs[k].isOpen = status); db.collection("settings").doc("booking").update(bookingSettings).then(() => addLog("All Status Changed")); }
function backupAndClearAll() { if(!confirm("전체 삭제? / Clear all?")) return; db.collection("slots").get().then(snap => { var batch = db.batch(); snap.forEach(doc => batch.delete(doc.ref)); batch.commit().then(() => { addLog("Cleared"); alert("Done"); }); }); }
...
function openReserveModal() { 
    ...
    if(m) { 
        ...
        document.getElementById("playerId").value = mine.playerId || ""; // ID 자동 완성
        document.getElementById("cancelKey").value = mine.cancelKey || ""; // 비밀번호 연속 완성
    } 
...
function deleteAttendee(slotId, index) {
    ...
    ref.get().then(doc => {
        ...
        ref.update({ attendees: list }).then(() => { addLog(`Admin deleted: ${pName}`); openReservedModal(slotId); });
    });
}
function openReserveFromStatus() { 
    if(!bookingSettings.tabs[currentBuff].isOpen && !adminAuthenticated) return alert("마감됨 / Locked.");
