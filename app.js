...
var langPack = {
    ko: {
        ...
        pAlliance: "연맹 (ZTP, BUG 등)", pNickname: "닉네임", pId: "ID (9자리)", pSpeed: "가속 일수", pPass: "비밀번호 (아무거나)",
...
function init() {
    ...
    window.db.collection("settings").doc("booking").onSnapshot(function(doc) { if(doc.exists) { bookingSettings = doc.data(); if(adminAuthenticated) fillAdminInputs(); } updateStatusMessage(); updateAdminUI(); window.renderAll(); });
