...
/* =====================================================================
   [필수 유틸리티 & 모달 오프너 최상단 배치] 
   에러 방지를 위해 호출되기 전에 모든 필수 함수가 정의되도록 위치 고정
   ===================================================================== */
...
window.openReserveFromStatus = function() { 
    if(!isTabActuallyOpen(currentBuff) && !adminAuthenticated) return; 
...
window.openReservedModal = function(id) { 
    ...
    if (!effectivelyOpen && !adminAuthenticated) { 
        ...
        displayList.forEach(function(a) { 
            ...
            // 본인 확인에 무관하게 어드민 또는 "취소" 팝업창을 띄우기 위한 용도이므로 모두에게 노출
...
window.confirmCancelAll = function() {
    ...
    ref.get().then(function(doc) {
        ...
        // [조건 해제] 이름 일치 여부는 무시하고 오직 패스워드만 일치하면 필터링하여 삭제
