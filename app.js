...
window.langPack = {
    ko: { 
        ...
        speedFree: "[현재 제한 없음] 누구나 자유롭게 예약 가능",
    ...
    en: { 
        ...
        speedFree: "[No Limits Currently] Free to book",
    ...
    fr: { 
        ...
        speedFree: "[Aucune limite] Libre de réserver",
    ...
    id: { 
        ...
        speedFree: "[Tidak Ada Batas] Bebas memesan",
    ...
    tr: { 
        ...
        speedFree: "[Şu An Sınır Yok] Serbest rezervasyon",
    ...
    ar: { 
        ...
        speedFree: "[لا توجد قيود حالياً] حجز حر للجميع",
...
window.getMinSpeedRequired = function() {
    if (window.bookingSettings.speedLimitEnforced === false) return 0;
...
window.applyLanguagePack = function() {
    ...
    if (noticeKoEl) { 
        ...
        var isEnforced = window.bookingSettings.speedLimitEnforced !== false;
        ...
        if (isEnforced) {
            ...
            var freeText = p.speedFree || "[No Limits Currently] Free to book";
            noticeKoEl.innerHTML = p.notice + "<br /><span style='color: #2ecc71; font-weight: bold;'>" + freeText + "</span>";
        }
    }
    ...
    safeSetPlaceholder("playerId", p.pId); safeSetPlaceholder("daysSaved", p.pSpeed); 
...
window.renderAll = function() {
    ...
    for (var h = 0; h < 24; h++) {
        for (var m = 0; m < 60; m += 30) {
            ...
            var isMine = attendees.some(function(a) { return window.normalizeText(a.player) === myName; });
...
window.updateAdminUI = function() { 
    ...
    var limitInd = document.getElementById("ind-limit-enforce");
    if (limitInd) {
        var isEnforced = window.bookingSettings.speedLimitEnforced !== false;
        if (isEnforced) {
            limitInd.textContent = "ON"; limitInd.style.background = "#2ecc71"; limitInd.style.color = "white";
        } else {
            limitInd.textContent = "OFF"; limitInd.style.background = "#e2e8f0"; limitInd.style.color = "#718096";
        }
    }
};
...
window.toggleSpeedLimitEnforce = function() {
    if (!window.db) return;
    var currentStatus = window.bookingSettings.speedLimitEnforced !== false; 
    window.db.collection("settings").doc("booking").update({ speedLimitEnforced: !currentStatus }).then(function() {
        window.addAdminLog("가속 제한 강제 적용: " + (!currentStatus ? "ON" : "OFF"));
    });
};
...
window.openReservedModal = function(id) { 
    ...
    if (!effectivelyOpen && !window.adminAuthenticated) { 
        ...
        displayList.forEach(function(a) { 
            ...
            starBtn.title = p.pinBooking || "Pin as My Booking";
            starBtn.onclick = function() {
                if (isStarred) {
                    localStorage.removeItem("starred_player");
                } else {
                    localStorage.setItem("starred_player", window.normalizeText(a.player));
                }
                window.openReservedModal(id);
                window.renderAll();
            };
            textSpan.appendChild(starBtn);
            
            var nameNode = document.createTextNode("[" + String(a.alliance).toUpperCase() + "] " + a.player);
            ...
            if (window.adminAuthenticated) { 
                ...
                editBtn.onclick = function() { window.adminEditSpecificBooking(id, a.id, a.daysSaved); }; 
...
window.confirmAdminLogin = function() {
    ...
    if(p === "2737") {
        ...
        var errorMsg = window.currentLang === 'ko' ? "비밀번호가 일치하지 않습니다." : "Invalid Password.";
        if(errEl) {
            errEl.innerText = errorMsg;
        } else {
            window.openCustomAlert(errorMsg);
        }
...
window.confirmBooking = function() {
    ...
    var newEntry = { 
        ...
        player: nickname, 
        playerId: idNum, 
        playerNormalized: window.normalizeText(nickname), 
        daysSaved: Number(d), 
        passwordHash: window.simpleHash(pass), 
        rawPassword: pass, 
        createdAt: Date.now() 
    };
...
window.init = function() {
    ...
    window.db.collection("settings").doc("booking").onSnapshot(function(doc) { 
        if(doc.exists) { 
            ...
            window.bookingSettings.speedLimitEnforced = data.speedLimitEnforced === true;
