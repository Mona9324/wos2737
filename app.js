...
var langPack = {
    ko: { 
        ...
        cancelLabel: "비밀번호 입력", 
        cancelBtn: "예약 취소", // 큰 버튼 전용
        addBookingBtn: "수정", // 큰 버튼 전용 (기존 예약추가에서 수정으로 명칭 변경)
    ...
    en: { 
        ...
        addTitle: "New Booking", confirmBtn: "Confirm", closeBtn: "Close", statusTitle: "Booking Status", cancelLabel: "Enter Password", 
    ...
    ja: { 
        ...
        curvedTxt: "予約サイトの利用料は : Monaの島 💚+1", confirmedHeader: "👑 確定した大統領バフ時間", addAlarm: "🔔 アラーム登録", mon: "月曜日", tue: "火曜日", thu: "木曜日", mondayShort: "月曜日", tuesdayShort: "火曜日", thursdayShort: "木曜日", optAll: "すべて", optMine: "自分の予約", openAvailable: "✅ 予約受付中", openClosed: "🔒 予約終了", pers: "人", noRes: "予約可能", addTitle: "新規予約追加", confirmBtn: "確定", closeBtn: "閉じる", statusTitle: "予約状況", cancelLabel: "パスワード入力", cancelBtn: "予約取消", addBookingBtn: "修正", closedAlert: "締め切られました。", speedUnit: "日", pAlliance: "同盟 (ZYZ, BUG, ZTP)", pNickname: "名前", pId: "プレイヤーID", pSpeed: "加速日数", pPass: "パスワード", editBtn: "修正", cancelBtnSmall: "取消", delBtn: "削除", slotOpenBtn: "🔓 開く", slotCloseBtn: "🔒 閉じる", errFill: "パスワードを入力してください。", errWrongPass: "パスワードが間違っています。", errNoRes: "予約データが見つかりません。", errFillAll: "すべての項目を入力してください。", errIdDigit: "プレイヤーIDは9桁の数字。", promptEdit: "修正:", errNan: "不正。", promptDelete: "削除？", promptClear: "すべての予約データを削除しますか？<br />（この操作はログに記録されます）", btnAdminDel: "🚨 全ての予約を削除", promptSaved: "保存されました！",
...
window.openReservedModal = function(id) { 
    ...
    if (!effectivelyOpen && !adminAuthenticated) { 
        ...
        displayList.forEach(function(a) { 
            ...
            // [UI 완전 개정 완료] 잔단추 그룹 삭제 후 관리용 강제 삭제만 어드민일 때 예외 노출
...
window.addAdminLog = function(msg) {
    if(!window.db) return;
    var now = new Date();
    var timeStr = "[" + now.getFullYear() + "-" + String(now.getMonth()+1).padStart(2,'0') + "-" + String(now.getDate()).padStart(2,'0') + " " + String(now.getHours())+ ":" + String(now.getMinutes()).padStart(2,'0') + "]";
