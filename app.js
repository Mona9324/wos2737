...
function confirmCancel() {
    ...
    if(!m) return alert("예약 정보를 찾을 수 없습니다. / No booking info found.");
...
// [수정] 관리자 및 설정 팝업 한/영 병기
function handleAdminAccess() { sc++; if(sc>=3) { sc=0; var p=prompt("Admin Password:"); if(p==="2737") { adminAuthenticated=true; document.getElementById("adminPanel").classList.add("show"); fillAdminInputs(); updateAdminUI(); addLog("Admin Login"); } } }
...
function backupAndClearAll() { if(!confirm("정말 전체 삭제하시겠습니까? 복구할 수 없습니다. / Are you sure you want to clear all data? This cannot be undone.")) return; db.collection("slots").get().then(snap => { var batch = db.batch(); snap.forEach(doc => batch.delete(doc.ref)); batch.commit().then(()=> { addLog("DB Cleared"); alert("전체 삭제 완료! / All data cleared!"); }); }); }
function deleteAttendee(slotId, index) { if(!confirm("이 예약을 삭제하시겠습니까? / Delete this entry?")) return; var ref = db.collection("slots").doc(slotId); ref.get().then(doc => { var list = doc.data().attendees; list.splice(index, 1); ref.update({ attendees: list }).then(() => { addLog("Entry Deleted by Admin"); openReservedModal(slotId); }); }); }
