...
function isTabActuallyOpen(day) {
    ...
    if (gOpen && now < gOpen) return false;
    if (dClose && now > dClose) return false;
    return true;
}
...
function saveAutoSchedule() {
    ...
    db.collection("settings").doc("booking").update(bookingSettings).then(() => { addLog("Auto Schedule Saved"); alert("저장됨 / Saved!"); });
}
