...
function confirmBooking() {
    ...
    // 가속 일수(d)를 포함한 모든 항목 필수 체크
    if(!a || !p || !idNum || !d || !pass) {
        return alert("모든 항목을 입력해야 예약이 가능합니다. / Please fill all fields including Speed-up Days.");
    }
    
    if(idNum.length !== 9 || isNaN(idNum)) {
        return alert("ID는 반드시 9자리 숫자여야 합니다. / ID must be 9 digits.");
    }
